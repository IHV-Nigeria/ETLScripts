...
def export_data(cutoff_datetime=None, filename=None):
    ...
    for doc in tqdm(cursor, total=size, desc="Nas VL ETL Progress"):
        ...
        last_marital_status = hivenrollmentutils.get_last_marital_status_obs(doc, cutoff_datetime).get(
            'variableValue') if hivenrollmentutils.get_last_marital_status_obs(doc, cutoff_datetime) else None
        last_occupational_status = hivenrollmentutils.get_last_occupational_status_obs(doc, cutoff_datetime).get(
            'variableValue') if hivenrollmentutils.get_last_occupational_status_obs(doc, cutoff_datetime) else None
        date_diagnosed = obsutils.getValueDatetimeFromObs(
            hivenrollmentutils.get_last_date_confirmed_hiv_positive_obs(doc,
                                                                        cutoff_datetime)) if hivenrollmentutils.get_last_date_confirmed_hiv_positive_obs(
            doc, cutoff_datetime) else None

        baseline_vl = labutils.get_first_viral_load_obs(doc, cutoff_datetime).get(
            'variableValue') if labutils.get_first_viral_load_obs(doc, cutoff_datetime) else None
        baseline_vl_date = commonutils.normalize_datetime(labutils.get_first_viral_load_obs(doc, cutoff_datetime).get(
            'obsDatetime')) if labutils.get_first_viral_load_obs(doc, cutoff_datetime) else None

        last_who_staging = carecardutils.get_last_who_stage_obs(doc, cutoff_datetime).get(
            'variableValue') if carecardutils.get_last_who_stage_obs(doc, cutoff_datetime) else None
        last_tb_status = carecardutils.get_current_tb_status_obs(doc, cutoff_datetime).get(
            'variableValue') if carecardutils.get_current_tb_status_obs(doc, cutoff_datetime) else None

        current_viral_load_obs = labutils.get_last_viral_load_obs_before(doc, cutoff_datetime)
        current_vl = obsutils.getValueNumericFromObs(current_viral_load_obs) if obsutils.getValueNumericFromObs(
            current_viral_load_obs) else None
        suppression_status = (1 if current_vl < 1000 else 0) if current_vl is not None else None

        first_suppressed_viral_load_obs = labutils.get_first_suppressed_viral_load_between_dates(doc, start_datetime,
                                                                                                 end_datetime)
        first_suppressed_viral_load_value = obsutils.getValueNumericFromObs(
            first_suppressed_viral_load_obs) if first_suppressed_viral_load_obs else None
        first_suppressed_viral_load_datetime = obsutils.getObsDatetimeFromObs(
            first_suppressed_viral_load_obs) if first_suppressed_viral_load_obs else None

        adherence_obs = carecardutils.get_nth_drug_adherence_obs_of_last_x_viral_loads(doc, 1, 1,
                                                                                       cutoff_datetime) if carecardutils.get_nth_drug_adherence_obs_of_last_x_viral_loads(
            doc, 1, 1, cutoff_datetime) else None
        ...
        record = {
            ...
            "ViralLoadSampleDate": obsutils.getValueDatetimeFromObs(
                labutils.get_sample_collection_date_obs_of_viral_load_obs(doc, current_viral_load_obs)),
            "ViralLoadIndication": obsutils.getVariableValueFromObs(
                labutils.get_viral_load_indication_obs_of_viral_load_obs(doc, current_viral_load_obs)),
            ...
            "TimeToSuppression (months)": commonutils.get_month_diff(first_suppressed_viral_load_datetime,
                                                                     date_diagnosed),
