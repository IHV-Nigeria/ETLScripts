...
from legacy.constants import pmtct_hts_register_concepts, general_antenatal_care_concepts

pmtct_hts_register_form_id = pmtct_hts_register_concepts.get("FORM_ID")
general_antenatal_care_form_id = general_antenatal_care_concepts.get("FORM_ID")
...
def initialize_eac_line_list_data(cutoff_datetime=None):
    ...
    try:
        #extracted_results = []
        for doc in tqdm(cursor, total=size, desc="PMTCT Line List ETL Progress"):
...
def convert_doc_to_record(doc, cutoff_datetime):
    ...
    Weight_loss_obs = obsutils.get_last_obs_before_date(doc, form_id= 54, concept_id= pmtct_hts_register_concepts.get("Weight loss"), cutoff_datetime=cutoff_datetime)
    Night_sweats_obs = obsutils.get_last_obs_before_date(doc, form_id= 54, concept_id= pmtct_hts_register_concepts.get("Night sweats"), cutoff_datetime=cutoff_datetime)
    Fever_obs = obsutils.get_last_obs_before_date(doc, form_id= 54, concept_id= pmtct_hts_register_concepts.get("Fever"), cutoff_datetime=cutoff_datetime)
    Cough_obs = obsutils.get_last_obs_before_date(doc, form_id= 54, concept_id= pmtct_hts_register_concepts.get("Cough"), cutoff_datetime=cutoff_datetime)
    Result_of_HIV_test_obs = obsutils.get_last_obs_before_date(doc, form_id= 54, concept_id= pmtct_hts_register_concepts.get("Result of HIV test"), cutoff_datetime=cutoff_datetime)
    HIV_Test_accepted_obs = obsutils.get_last_obs_before_date(doc, form_id=54, concept_id=pmtct_hts_register_concepts.get("HIV Test accepted"), cutoff_datetime=cutoff_datetime)
    Received_HIV_test_result_obs = obsutils.get_last_obs_before_date(doc, form_id=54, concept_id=pmtct_hts_register_concepts.get("Received HIV test result"), cutoff_datetime=cutoff_datetime)
    Agreed_To_Partner_Notification_obs = obsutils.get_last_obs_before_date(doc, form_id=54, concept_id=pmtct_hts_register_concepts.get("Agreed To Partner Notification"), cutoff_datetime=cutoff_datetime)
    TB_Screening_Score_obs = obsutils.get_last_obs_before_date(doc, form_id=54, concept_id=pmtct_hts_register_concepts.get("TB Screening Score"), cutoff_datetime=cutoff_datetime)
    HTS_Register_Date_obs = obsutils.get_last_obs_before_date(doc, form_id=54, concept_id=pmtct_hts_register_concepts.get("HTS Register Date"), cutoff_datetime=cutoff_datetime)
    Previously_Known_HIV_Positive_Result_obs = obsutils.get_last_obs_before_date(doc, form_id=54, concept_id=pmtct_hts_register_concepts.get("Previously Known HIV Positive Result"), cutoff_datetime=cutoff_datetime)

    VDRL_obs = obsutils.get_last_obs_before_date(doc, form_id=16, concept_id= general_antenatal_care_concepts.get("VDRL"), cutoff_datetime=cutoff_datetime)
    Date_Tested_For_Syphilis_obs = obsutils.get_last_obs_before_date(doc, form_id=16, concept_id= general_antenatal_care_concepts.get("Date Tested For Syphilis"), cutoff_datetime=cutoff_datetime)
    Tested_for_Syphilis_obs = obsutils.get_last_obs_before_date(doc, form_id=16, concept_id= general_antenatal_care_concepts.get("Tested for Syphilis"), cutoff_datetime=cutoff_datetime)
    General_Antenatal_Number_obs = obsutils.get_last_obs_before_date(doc, form_id=16, concept_id= general_antenatal_care_concepts.get("General Antenatal Number"), cutoff_datetime=cutoff_datetime)
    ...
    record = {
        ...
        "Weight loss": Weight_loss_obs.get("variableValue") if Weight_loss_obs else None,
        "Night sweats": Night_sweats_obs.get("variableValue") if Night_sweats_obs else None,
        "Fever": Fever_obs.get("variableValue") if Fever_obs else None,
        "Cough": Cough_obs.get("variableValue") if Cough_obs else None,
        "Result of HIV test": Result_of_HIV_test_obs.get("variableValue") if Result_of_HIV_test_obs else None,
        "HIV Test accepted": HIV_Test_accepted_obs.get("variableValue") if HIV_Test_accepted_obs else None,
        "Received HIV test result": Received_HIV_test_result_obs.get("variableValue") if Received_HIV_test_result_obs else None,
        "Agreed To Partner Notification": Agreed_To_Partner_Notification_obs.get("variableValue") if Agreed_To_Partner_Notification_obs else None,
        "TB Screening Score": TB_Screening_Score_obs.get("variableValue") if TB_Screening_Score_obs else None,
        "HTS Register Date": obsutils.getValueDatetimeFromObs(HTS_Register_Date_obs) if HTS_Register_Date_obs else None,
        "Previously Known HIV Positive Result": Previously_Known_HIV_Positive_Result_obs.get("variableValue") if Previously_Known_HIV_Positive_Result_obs else None,

        "VDRL": VDRL_obs.get("variableValue") if VDRL_obs else None,
        "Date Tested For Syphilis": obsutils.getValueDatetimeFromObs(Date_Tested_For_Syphilis_obs) if Date_Tested_For_Syphilis_obs else None,
        "Tested for Syphilis": Tested_for_Syphilis_obs.get("variableValue") if Tested_for_Syphilis_obs else None,
        "General Antenatal Number": General_Antenatal_Number_obs.get("variableValue") if General_Antenatal_Number_obs else None
...
def pmtct_data(cutoff_datetime=None, filename=None):
    db_name = MONGO_DATABASE_NAME
    db = mongo_dao.get_db_connection(db_name)
    cursor, size = mongo_dao.get_pmtct_containers(db, db_name)

    # Prepare the file path (create directory and name)
    ...
    cutoff_datetime = commonutils.normalize_clinical_date(datetime(2025, 10, 1)) if cutoff_datetime else None

    start_datetime = legacy.constants.FY26_START_DATE
    end_datetime = commonutils.normalize_clinical_date(datetime.now())
    ...
    #extracted_results = []
    for doc in tqdm(cursor, total=size, desc="PMTCT ETL Progress"):
        ...
        facility_info = get_facility_by_datim(datim_code)
        art_start_date = commonutils.normalize_clinical_date(artcommence.get_art_start_date(doc, cutoff_datetime))
        pregnancy_status_obs = obsutils.get_last_obs_before_date(doc, form_id=legacy.constants.CARE_CARD_FORM_ID, concept_id= legacy.constants.PREGNANCY_STATUS_CONCEPT_ID , cutoff_datetime=cutoff_datetime)
        pregnancy_status = obsutils.getVariableValueFromObs(pregnancy_status_obs) if pregnancy_status_obs else None
        edd_obs = obsutils.get_last_obs_before_date(doc, form_id=legacy.constants.CARE_CARD_FORM_ID, concept_id= legacy.constants.EDD_CONCEPT_ID , cutoff_datetime=cutoff_datetime)
        edd = obsutils.getValueDatetimeFromObs(edd_obs) if edd_obs else None
        last_pregnant_pregnancy_status_obs = obsutils.get_last_pregnant_obs(doc, cutoff_datetime=cutoff_datetime)
        last_pregnant_pregnancy_status = obsutils.getVariableValueFromObs(last_pregnant_pregnancy_status_obs) if last_pregnant_pregnancy_status_obs else None
        last_pregnant_pregnancy_status_date = obsutils.getObsDatetimeFromObs(last_pregnant_pregnancy_status_obs) if last_pregnant_pregnancy_status_obs else None
        pregnant_edd_obs = obsutils.get_obs_with_encounter_id(doc, encounter_id=last_pregnant_pregnancy_status_obs.get("encounterId"), concept_id= legacy.constants.EDD_CONCEPT_ID) if last_pregnant_pregnancy_status_obs else None
        pregnant_edd = obsutils.getValueDatetimeFromObs(pregnant_edd_obs) if pregnant_edd_obs else None


        last_pmtct_hts_encounter_date = encounterutils.get_encounter_datetime(encounterutils.get_last_pmtct_hts_encounter(doc, cutoff_datetime=cutoff_datetime))
        Weight_loss_obs = obsutils.get_last_obs_before_date(doc, form_id= 54, concept_id= pmtct_hts_register_concepts.get("Weight loss"), cutoff_datetime=cutoff_datetime)
        Night_sweats_obs = obsutils.get_last_obs_before_date(doc, form_id= 54, concept_id= pmtct_hts_register_concepts.get("Night sweats"), cutoff_datetime=cutoff_datetime)
        Fever_obs = obsutils.get_last_obs_before_date(doc, form_id= 54, concept_id= pmtct_hts_register_concepts.get("Fever"), cutoff_datetime=cutoff_datetime)
        Cough_obs = obsutils.get_last_obs_before_date(doc, form_id= 54, concept_id= pmtct_hts_register_concepts.get("Cough"), cutoff_datetime=cutoff_datetime)
        ...
        last_general_antenatal_date = encounterutils.get_encounter_datetime(encounterutils.get_last_general_antenatal_encounter(doc, cutoff_datetime=cutoff_datetime))
        VDRL_obs = obsutils.get_last_obs_before_date(doc, form_id=16, concept_id= general_antenatal_care_concepts.get("VDRL"), cutoff_datetime=cutoff_datetime)
        Date_Tested_For_Syphilis_obs = obsutils.get_last_obs_before_date(doc, form_id=16, concept_id= general_antenatal_care_concepts.get("Date Tested For Syphilis"), cutoff_datetime=cutoff_datetime)
        Tested_for_Syphilis_obs = obsutils.get_last_obs_before_date(doc, form_id=16, concept_id= general_antenatal_care_concepts.get("Tested for Syphilis"), cutoff_datetime=cutoff_datetime)
        General_Antenatal_Number_obs = obsutils.get_last_obs_before_date(doc, form_id=16, concept_id= general_antenatal_care_concepts.get("General Antenatal Number"), cutoff_datetime=cutoff_datetime)
        ...
        record = {
            ...
            "LastPickupDate": pharmacyutils.get_last_arv_pickup_date(doc,cutoff_datetime),
            "LastVisitDate": encounterutils.get_last_encounter_date(doc,cutoff_datetime),
            "Last Visit Pregnancy Status": pregnancy_status,

            "Last Pregnant Pregnancy Status": last_pregnant_pregnancy_status,
            "Last Pregnant Pregnancy Status Date": last_pregnant_pregnancy_status_date,
            "Last Pregnant Pregnancy EDD": pregnant_edd,

            "Last PMTCT Encounter": last_pmtct_hts_encounter_date,
            "Weight loss": Weight_loss_obs.get("variableValue") if Weight_loss_obs else None,
            "Night sweats": Night_sweats_obs.get("variableValue") if Night_sweats_obs else None,
            "Fever": Fever_obs.get("variableValue") if Fever_obs else None,
            "Cough": Cough_obs.get("variableValue") if Cough_obs else None,
            ...
            "Last General Antenatal Encounter": last_general_antenatal_date,
            "VDRL": VDRL_obs.get("variableValue") if VDRL_obs else None,
            "Date Tested For Syphilis": obsutils.getValueDatetimeFromObs(Date_Tested_For_Syphilis_obs) if Date_Tested_For_Syphilis_obs else None,
            "Tested for Syphilis": Tested_for_Syphilis_obs.get("variableValue") if Tested_for_Syphilis_obs else None,
            "General Antenatal Number": General_Antenatal_Number_obs.get("variableValue") if General_Antenatal_Number_obs else None
...
def prepare_filepath(filename=None):
    ...
    if filename:
        ...
        filename = f"PMTCTExport_{timestamp}.csv"
